...
@interface ALDataBasePool()

- (void)pushDatabaseBackInPool:(ALDataBase*)db;
- (ALDataBase*)db;

@end


@implementation ALDataBasePool
...
- (void)pushDatabaseBackInPool:(ALDataBase*)db {
  ...
  [self executeLocked:^() {
    ...
    if ([self->_databaseInPool containsObject:db]) {
      [[NSException exceptionWithName:@"Database already in pool" reason:@"The ALDataBase being put back into the pool is already present in the pool" userInfo:nil] raise];
    }
...
- (ALDataBase*)db {
  ...
  [self executeLocked:^() {
    ...
    if (db) {
      ...
      db = [ALDataBase databaseWithPath:self->_path];
