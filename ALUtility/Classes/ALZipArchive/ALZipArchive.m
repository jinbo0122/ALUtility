...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(id<ALZipArchiveDelegate>)delegate {
	return [self unzipFileAtPath:path toDestination:destination overwrite:YES password:nil error:nil delegate:delegate];
}


+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<ALZipArchiveDelegate>)delegate {
	// Begin opening
	zipFile zip = PDunzOpen((const char*)[path UTF8String]);
	if (zip == NULL) {
		NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"failed to open zip file" forKey:NSLocalizedDescriptionKey];
		if (error) {
			*error = [NSError errorWithDomain:@"ALZipArchiveErrorDomain" code:-1 userInfo:userInfo];
		}
		return NO;
	}
	...
	PDunzGetGlobalInfo(zip, &globalInfo);

	// Begin unzipping
	if (PDunzGoToFirstFile(zip) != UNZ_OK) {
		...
		if (error) {
			*error = [NSError errorWithDomain:@"ALZipArchiveErrorDomain" code:-2 userInfo:userInfo];
		}
		return NO;
	}
	...
	do {
		@autoreleasepool {
			if ([password length] == 0) {
				ret = PDunzOpenCurrentFile(zip);
			} else {
				ret = PDunzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSASCIIStringEncoding]);
			}
			...
			ret = PDunzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
			if (ret != UNZ_OK) {
				success = NO;
				PDunzCloseCurrentFile(zip);
				break;
			}
			...
			PDunzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
	        ...
	        if (nil != err) {
	            NSLog(@"[ALZipArchive] Error: %@", err.localizedDescription);
	        }
			...
			if(!fileIsSymbolicLink)
	        {
	            ...
	            while (fp) {
	                int readBytes = PDunzReadCurrentFile(zip, buffer, 4096);
	            ...
	            if (fp) {
	                ...
	                if (fileInfo.dosDate != 0) {
	                    ...
	                    if (attr) {
	                        if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
	                            ...
	                            NSLog(@"[ALZipArchive] Failed to set attributes - whilst setting modification date");
	                        }
	                    }
	                }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[ALZipArchive] Failed to set attributes - whilst setting permissions");
                        }
            ...
            else
            {
                ...
                int bytesRead = 0;
                while((bytesRead = PDunzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            NSLog(@"[ALZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            NSLog(@"[ALZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths {
	BOOL success = NO;
	ALZipArchive *zipArchive = [[ALZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
...
- (BOOL)open {
	...
	_zip = PDzipOpen([_path UTF8String], APPEND_STATUS_CREATE);
	return (NULL != _zip);
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName {
    ...
    PDzipOpenNewFileInZip(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    ...
    while (!feof(input))
    {
		...
		PDzipWriteInFileInZip(_zip, buffer, len);
	}

	PDzipCloseFileInZip(_zip);
	free(buffer);
	fclose(input);
	return YES;
}
...
- (BOOL)writeData:(NSData *)data filename:(NSString *)filename {
	...
	PDzipOpenNewFileInZip(_zip, [filename UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);

    PDzipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close {
	NSAssert((_zip != NULL), @"[ALZipArchive] Attempting to close an archive which was never opened");
	PDzipClose(_zip, NULL);
	return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime {
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[ALZipArchive] MSDOS date masks don't add up");
