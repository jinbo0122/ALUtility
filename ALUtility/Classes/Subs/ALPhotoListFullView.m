...
#ifndef BBSDKMODE
#if !defined(SCAppVersion) && !defined(MSAppVersion) && !defined(BAAppVersion) && !defined(BWAppVersion) && !defined(XHAppVersion)
#import "../../Pandora/Pandora/Pandora/Classes/Service/Logic/Categories/Image/UIImageView+Utility.h"
#else
...
-(void)imageViewPressLongGestureAction:(UILongPressGestureRecognizer*)longPress{
  if (longPress.state == UIGestureRecognizerStateBegan) {
    ...
    [[[UIActionSheet alloc] initWithTitle:@"保存图片到手机相册"
                         cancelButtonItem:[ALButtonItem itemWithLabel:@"取消"]
                    destructiveButtonItem:[ALButtonItem itemWithLabel:@"确定" action:^{
      [wSelf.imageView.image saveToAlbum:bundleDisplayName
                         ...
                         otherButtonItems:nil] showInView:self];
  }
}
